...
%%%%%%%%%%%%%%%%%%%%%%%
% Associa al moderador un dia on tingui un event
assignManED:-
...
% un moderador no pot tenir assignats mes dies dels permesos
eachMmaxD:-
    moderator(M), maxDaysPerModerator(MAX),
    findall(md(M, D), day(D), Dies),
    atMost(MAX, Dies),
    fail.
eachMmaxD.
% un dia no pot tenir mes events del maxim permes
eachDmaxE:-
    day(D), maxEventsPerDay(MAX),
    findall(ed(E, D), event(E), Events),
    atMost(MAX, Events),
    fail.
eachDmaxE.
% cada event ha de tenir assignat un dia
eachEoneD:-
    event(E),
    eventDays(E, DS),
    findall(ed(E, D), member(D, DS), Dies),
    exactly(1, Dies),
    fail.
eachEoneD.
% cada event ha de tenir un sol moderador
eachEoneM:-
    event(E),
    eventModerators(E, MS),
    findall(em(E, M), member(M, MS), Moderadors),
    exactly(1, Moderadors),
    fail.
eachEoneM.
