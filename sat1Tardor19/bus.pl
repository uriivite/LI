...
satVariable( bdcc(B, D, C1, C2) ):- bus(B), day(D), city(C1), city(C2).

writeClauses:-
    defineTrip,
    eachC1C2atLeastOneTrip,
    onlyValidTrips,
    forceTrip,
    eachDBoneC,
    eachDConeB,
...
eachC1C2atLeastOneTrip:-
    trip(C1-C2),
    findall(bdcc(B, D, C1, C2), (bus(B), day(D), notLastDay(D)), Viatges),
    atLeast(1, Viatges),
    fail.
eachC1C2atLeastOneTrip.

defineTrip:-
    bus(B), city(C1), city(C2),
    day(D1), notLastDay(D1), consecutiveDays(D1, D2),
    expressAnd(bdcc(B, D1, C1, C2), [bdc(B, D1, C1), bdc(B, D2, C2)]),
    fail.
defineTrip.

onlyValidTrips:-
    day(D2), notLastDay(D2), consecutiveDays(D1, D2, D3),
    bus(B), trip(C1-C2), trip(C2-C3),
    dist(C1, C2, DIST1), dist(C2, C3, DIST2), maxDist(MAX),
    DIST is DIST1 + DIST2, DIST > MAX,
    writeClause([-bdc(B, D1, C1), -bdc(B, D2, C2), -bdc(B, D3, C3)]),
    fail.
onlyValidTrips.
    

% donats dos dies consecutius, un bus ha de ser a dues ciutats diferents
forceTrip:-
    day(D1), day(D2), notLastDay(D1), consecutiveDays(D1, D2), city(C),
    writeClause([-bdc(B, D1, C), -bdc(B, D2, C)]),
    fail.
forceTrip.
% cada dia, un bus nomes pot estar a una ciutat
eachDBoneC:-
    day(D), bus(B),
    findall(bdc(B, D, C), city(C), Ciutats),
    exactly(1, Ciutats),
    fail.
eachDBoneC.
% cada dia, a cada ciutat ha d haver un bus
eachDConeB:-
    day(D), city(C),
    findall(bdc(B, D, C), bus(B), Busos),
    exactly(1, Busos),
    fail.
eachDConeB.
