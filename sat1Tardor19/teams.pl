...
incompatibleWorkers(W1,W2):-
    workerScore(W1, S1), workerScore(W2, S2),
    SUM is S1 + S2,
    maxScore(MAX),
    SUM > MAX.
...
%%%%%%%%%%%%%%%%%%%%%%%
% dos treballadors incompatibles no poden estar al mateix equip
divideIncompW:-
    worker(W1), worker(W2), W1 \= W2, team(T),
    incompatibleWorkers(W1, W2),
    writeClause([-wt(W1, T), -wt(W2, T)]),
    fail.
divideIncompW.

% Cada treballador ha d estar a un equip
eachWoneT:-
    worker(W),
    findall(wt(W, T), team(T), Teams),
    exactly(1, Teams),
    fail.
eachWoneT.

% un equip a de tenir com a minim el nombre de treballadors permes
eachTminW:-
    team(T), minSize(MIN),
    findall(wt(W, T), worker(W), Workers),
    atLeast(MIN, Workers),
    fail.
eachTminW.

% un equip no pot tenir mes treballadors que el maxim permes
eachTmaxW:-
    team(T), maxSize(MAX),
    findall(wt(W, T), worker(W), Workers),
    atMost(MAX, Workers),
